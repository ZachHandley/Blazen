# Changelog

## Unreleased

### Added — Telemetry: OTLP/HTTP + OpenInference spans (BLAZEN_IMPROVEMENTS §11)

- **`OtlpConfig` gains runtime protocol selection + auth headers.** New `OtlpConfig.protocol` field (`OtlpProtocol::Grpc | HttpProto`, default `HttpProto`) and `OtlpConfig.headers: Option<HashMap<String, String>>`. The public `init_otlp` is now a dispatcher that picks the gRPC (`otlp` feature) or HTTP/binary-protobuf (`otlp-http` feature) exporter based on `protocol`, so a single entrypoint serves both transports. Headers are honored on HTTP (Honeycomb `x-honeycomb-team`, OTLP/HTTP `Authorization`, etc.); on gRPC they are dropped with a `tracing::warn!` (Blazen carries no direct `tonic` dep for metadata-map construction — use HTTP for header-based auth). Existing 0.5.x configs deserialize unchanged via `#[serde(default)]` on the new fields. The `otlp-http` Cargo feature is now forwarded through `blazen-py`, `blazen-uniffi`, and `blazen-cabi` (Node already had it).
  - **Bindings** — `OtlpConfig.protocol` / `headers` exposed across Python (`OtlpProtocol` enum + kwargs), Node (`OtlpConfig.protocol` + the previously-inert `headers` field, now forwarded), UniFFI (Go `InitOTLPWithOptions` + `OtlpProtocol`; Swift `initOtlp(…, protocol:headers:)`), and cabi (`blazen_init_otlp` gains `protocol: i32` + header key/value arrays, surfaced as Ruby `init_otlp(protocol:, headers:)`). WASM stays HTTP-only (`OtlpConfig.setHeaders({...})`; tonic does not compile to wasm32).
- **`wrap_with_tracing` spans are now eval-grade.** `TracingModel` emits OpenInference + `gen_ai.*` aliases (`openinference.span.kind = "LLM"`, `gen_ai.system`, `gen_ai.request.model`, `gen_ai.usage.input_tokens` / `output_tokens`, `gen_ai.response.finish_reasons`, `llm.token_count.*`, `llm.model_name`) alongside the existing Blazen-flavored attributes — so the same span is ingestable by PostHog AI and eval-grade in Phoenix Arize without post-processing. New `TracingConfig::with_message_capture(bool)` (default off, privacy-safe) opts into recording raw prompt + completion text as `llm.input_messages` / `llm.output_messages`. Exposed as `wrap_with_tracing(model, provider_name, capture_messages=False)` (Python) and `model.withTracing(name, captureMessages?)` (Node).

### Breaking changes — Provider class refactor (Waves A–E)

- **Central `*Model` opaques deleted across all 9 surfaces.** Replaced with the per-engine provider hierarchy: `BaseProvider → <Capability>Provider → <Engine>Provider`. Consumers instantiate the engine class directly and pass it to `Agent` / `Batch` / streaming entry points.
  - **Rust / UniFFI** — `Model`, `EmbeddingModel`, `TtsModel`, `SttModel`, `ImageGenModel`, `ThreeDModel`, `MusicModel`, `VcModel` and the 27 `new_*_model` / `new_*_embedding_model` factories removed. `Agent::new` and `complete_batch[_blocking]` now take `Arc<dyn LlmProvider>` — every per-engine class implements the capability trait, so any of them coerces in. Central `complete_streaming[_blocking]` removed; use per-engine streaming free fns (`<engine>_provider_complete_streaming`).
  - **cabi** — `BlazenModel` / `BlazenEmbeddingModel` opaques + their factory C fns removed (`crates/blazen-cabi/src/llm.rs`, `providers.rs`, central media wrappers, central LLM streaming). New `BlazenLlmProvider` / `BlazenEmbeddingProvider` opaques wrap `Arc<dyn LlmProvider>` / `Arc<dyn EmbeddingProvider>`; each per-engine cabi class exposes `blazen_<engine>_provider_as_llm_provider` / `as_embedding_provider` conversion C fns (21 LLM, 5 embedding). `blazen_agent_new` and `blazen_complete_batch[_blocking]` now take `*const BlazenLlmProvider`.
  - **Ruby** — `Blazen::Llm::Model`, `Blazen::Llm::EmbeddingModel` and top-level `Blazen::Model` / `Blazen::EmbeddingModel` aliases removed. New `Blazen::LlmProvider` / `Blazen::EmbeddingProvider` polymorphic bases; per-engine classes ship `#as_llm_provider` / `#as_embedding_provider`. `Blazen::Agent.new(provider: …)` and `Blazen::Batch.complete(provider, …)` accept either a polymorphic base or any per-engine instance and auto-convert. `Blazen::Providers.openai / .anthropic / …` central factories were repointed to per-engine class constructors (call-shape preserved as a back-compat shim).
  - **Go** — `Model`, `EmbeddingModel`, `TtsModel`, `SttModel`, `ImageGenModel` Go wrappers and the `New*Model` / `New*Embedding` factories deleted. The hand-written `providers.go` and `compute.go` are removed entirely; `agent.go` and `batch.go` now take `*uniffiblazen.LlmProvider`. The decorator helpers `NewBaseProviderFromModel` / `NewBaseProviderWithDefaults` / `ProviderDefaultsOf` are gone; `LlmProviderDefaults` from the generated bindings is the replacement. Consumers can use the generated `uniffiblazen.<Engine>Provider` classes directly — the central-handle Go shim is no longer in the way.
  - **Swift** — `Model` / `EmbeddingModel` / `TtsModel` / `SttModel` / `ImageGenModel` typealiases and the central `Providers` namespace factories removed. `Agent` / `Batch` take `any UniFFIBlazen.LlmProvider`. Custom-provider chainable extensions migrated off `BaseProvider` onto `LlmProviderDefaults`. The `Compute` namespace shrank to the result DTOs.
  - **Kotlin** — `BaseProvider.systemPrompt` / `.toolsJson` / `.responseFormatJson` extension properties removed (they wrapped a now-deleted decorator method); use `LlmProviderDefaults` directly. All other hand-written Kotlin types were already DTO shadows and pick up the new surface automatically through the regenerated `dev.zorpx.blazen.uniffi.*` package.
  - **Python / Node / WASM** — these surfaces never depended on the central uniffi opaques and were untouched by this wave. The per-engine napi-rs / PyO3 / wasm-bindgen classes that already shipped in 0.5.x remain the canonical entry point.
- **Streaming model** — uniffi-side central streaming free fns (`complete_streaming`, `stream_generate_music_to_sink`, `stream_convert_pcm_to_sink` + `_blocking` variants) are gone. Each per-engine class exposes its own streaming free fn (LLM: `<engine>_provider_complete_streaming[_blocking]`; music: `<engine>_provider_stream_generate_music_to_sink[_blocking]`; VC: `<engine>_provider_stream_convert_pcm_to_sink[_blocking]`). Sink traits (`CompletionStreamSink`, `MusicStreamSink`, `VcStreamSink`) and the `drive_*_stream` helpers are unchanged.

### Fixed — Wave E foundation

- **cabi default-features build** — `crates/blazen-cabi/src/compute_records.rs` was importing `blazen_uniffi::compute::ThreeDGenerateResult` unconditionally even though the uniffi-side type is gated behind `triposr`. Default-features cabi builds now compile without `BLAZEN_UNIFFI_FEATURES=triposr` env override. The `BlazenThreeDGenerateResult` struct + impls + extern fns + tests are now all `#[cfg(feature = "triposr")]` — matching the per-engine cabi `blazen_triposr_provider_*` factories that produce these handles.
- **Go cgo `-lgomp`** — `bindings/go/cgo_link.go` + `cgo_link_linux_arm64.go` now pass `-lgomp` to the linker so the OpenMP-built `ggml-cpu` symbols in the all-features static archive resolve without an `CGO_LDFLAGS="-lgomp"` env override. Linux-only; Windows OpenMP linkage is unchanged.

### Fixed

- core: bound the `WorkflowHandler::result` accumulator drain with a 50 ms timeout-then-abort. Previously it awaited the broadcast channel's `Closed` state, which only arrives once every `Sender` clone (including those held by binding-side `Context` wrappers like napi-rs `JsContext`) is dropped. JS GC could keep `JsContext` alive arbitrarily long, leaving the accumulator awaiting a sender that never dropped — `await wf.run()` would hang in Node after the workflow completed cleanly. By the time the accumulator-drain runs, no further `UsageEvent`s can be emitted, so the bounded window is sufficient. Also bumped `napi` to 3.8.6 and `napi-derive` to 3.5.5.
- node: `CustomProvider` prototype-walk re-entrance bug. `JsCustomProviderAdapter::from_host_object` previously used `has_named_property`, which walks the full prototype chain — including the napi-installed `CustomProvider.prototype` methods. A subclass that overrode only `generateImage` would still have `textToSpeech` bound back into the napi base prototype, looping the dispatcher. Fixed by walking the prototype chain manually with `has_own_property` and stopping at the napi-installed prototype. Subclasses now correctly report Unsupported for un-overridden methods.
- go: replaced the misleading `TODO: this is bad - all synchronization from ObjectRuntime.go is discarded here…` blocks (19 occurrences) and `TODO: Remove this` (1 occurrence) in the generated UniFFI Go bindings with audited `SAFETY` / `Defensive` comments explaining the actual control flow. `incrementPointer` clones the Rust `Arc` before `Lower` returns, so the handle the C caller receives is independently refcounted on the Rust side and survives the Go-side `defer decrementPointer()`. The patch is applied automatically by `scripts/regen-bindings.sh`.
- ruby: `CustomProvider` subclasses can now return success values from typed-method overrides, not just raise. Previously the trampolines always returned `-1` because Ruby had no way to construct cabi result handles (`BlazenAudioResult *`, `BlazenImageResult *`, etc.) — overrides could only signal failure. Added `blazen_*_from_json` constructors in `blazen-cabi` for the 8 typed result records + a `BlazenVoiceHandleArray` opaque type + `blazen_error_from_json`. New `Blazen::AudioResult` / `ImageResult` / `VideoResult` / `ThreeDResult` / `TranscriptionResult` / `VoiceHandle` / `CompletionResult` / `EmbeddingResult` Ruby wrappers (in `bindings/ruby/lib/blazen/providers/results.rb`) take a Hash and call the matching cabi constructor. Trampolines in `bindings/ruby/lib/blazen/providers/custom_base.rb` now convert a returned `Hash` or `Blazen::*Result` into a cabi handle and write it through the vtable's `out_response`. Errors raised in the override map to a typed `BlazenError` variant (`Unsupported`, `Validation`, `Timeout`, `Provider`, `Auth`, `RateLimit`, `Internal`) via `blazen_error_from_json`. Closes the V1 limitation documented in the prior session.

### Changed

- node: `CustomProvider` constructor now takes an options object: `new CustomProvider({ providerId: "...", protocol?: ApiProtocol })`. Replaces the previous positional `(providerId, protocol?)` signature so the binding matches the rest of the Node surface (`CompletionModel`, `EmbeddingModel`, capability providers all use options objects). The TypeScript type is emitted via `ts_args_type` so subclassers get IntelliSense for `{ providerId, protocol }`.

### Added — Library finish-out (Waves 0–11)

- **Timeouts** — `Pipeline.total_timeout`, `StepRegistration.timeout` + `with_timeout`/`no_timeout` builders, `HttpClientConfig` with `request_timeout`/`connect_timeout`/`unlimited()`, applied in `ReqwestHttpClient` and the WASM `FetchHttpClient` (via `AbortController` race).
- **Configurable retry at every scope** — `RetryConfig` plumbed at Pipeline / Workflow / Step / Provider / per-call levels via `RetryStack` and `resolve_retry()`. Builder methods (`retry_config`, `no_retry`, `step_retry`, etc.) on Pipeline / Workflow / WorkflowBuilder. All 17 LLM providers carry a `retry_config: Option<Arc<RetryConfig>>` field with a `with_retry_config` builder. New decorators: `RetryEmbeddingModel`, `RetryHttpClient`, `RetryComputeProvider`, `RetryMemoryBackend`. `Context::resolved_retry`/`with_*_retry` API.
- **Cost / usage rollup** — `UsageEvent` typed event, `Modality` enum, `TokenUsage::add` (saturating-sum across all token fields). `PipelineState.usage_total/cost_total_usd`, `StageResult.usage/cost_usd`, `PipelineResult.usage_total/cost_total_usd`, `WorkflowResult.usage_total/cost_total_usd`. `Context::emit_usage`. `UsageRecordingCompletionModel` / `UsageRecordingEmbeddingModel` decorators with pluggable `UsageEmitter` trait. Pricing: `PricingEntry` extended with `per_image`/`per_second`; helpers `compute_image_cost`/`compute_audio_cost`/`compute_video_cost`. Media result types (`ImageResult`, `AudioResult`, `VideoResult`, `ThreeDResult`, `TranscriptionResult`) carry usage + per-modality quantity fields.
- **Typed shared state** — `PipelineState<S>` generic with `S = serde_json::Value` default. Typed accessors (`shared`, `shared_mut`) plus an `objects: HashMap<String, Box<dyn Any + Send + Sync>>` escape-hatch for non-Serialize values (snapshot-excluded). `PipelineBuilder::with_state::<S2>()` pivot. Snapshots remain JSON-bound via `S: Serialize + DeserializeOwned`.
- **Sub-workflow steps** — `StepKind::Regular | SubWorkflow | ParallelSubWorkflows`. `SubWorkflowStep` (with own state, timeout, retry) and `ParallelSubWorkflowsStep` (with `JoinStrategy::WaitAll | FirstCompletes`). `WorkflowBuilder::add_subworkflow_step` / `add_parallel_subworkflows`.
- **Exit tool** — built-in `finish_workflow` synthetic tool registered on every Agent (opt-out via `AgentConfig::no_finish_tool`, rename via `finish_tool_name`); user-marker `is_exit: true` on any `TypedTool` via `.exit_tool(true)` builder. Loop short-circuits on first exit-marked tool call.
- **Progress events** — `ProgressEvent { kind: Pipeline | Workflow | SubWorkflow | Stage, current, total, percent, label, run_id }` emitted on the broadcast stream before each pipeline stage and after each workflow step. `PipelineHandler::progress() -> ProgressSnapshot` accessor. `WorkflowBuilder::auto_publish_events` default flipped from `false` to `true`.
- **`http_client()` escape hatch** — every LLM provider exposes the underlying `Arc<dyn HttpClient>` via inherent method + trait method on `CompletionModel` / `EmbeddingModel` (default-impl `None`).
- **Bindings parity** — Python (`blazen-py`), Node (`blazen-node`), and WASM-SDK (`blazen-wasm-sdk`) wrappers added for every new public surface; typegens (`blazen.pyi`, `index.d.ts`, `blazen_wasm_sdk.d.ts`) regenerated. Python adds sync wrappers (`PyBlocking*`) for select async classes.

### Changed — Library finish-out (Waves 0–11)

- `WorkflowBuilder::auto_publish_events` default is now `true`. Tests asserting on emitted-event count have been updated.
- `PipelineState` no longer derives `Debug` / `Clone` (incompatible with `Box<dyn Any>` in the new `objects` bag).
- Internal `accumulate_usage` in `agent.rs` now sums every `TokenUsage` field (previously dropped `reasoning_tokens`, `cached_input_tokens`, audio token fields).

### Breaking changes — PR-AUDIO restructure (Waves 1–28)

- **Crate rename: `blazen-audio-whispercpp` → `blazen-audio-stt`** with `whispercpp` as a backend submodule (`blazen_audio_stt::backends::whispercpp::WhisperCppBackend`). The `whispercpp` cargo feature on `blazen-audio-stt` activates the backend.
- **Crate rename: `blazen-audio-piper` folded into `blazen-audio-tts`** as a backend (`blazen_audio_tts::backends::piper::PiperBackend`). The dedicated `blazen-audio-piper` crate is removed.
- **Crate split: `blazen-audio-candle` → `blazen-audio-music` + `blazen-audio-codec`.** MusicGen / AudioGen / Stable Audio land under `blazen-audio-music`; EnCodec / DAC / SNAC land under `blazen-audio-codec`. The combined `blazen-audio-candle` crate is removed.
- **NEW crate `blazen-audio`** — capability-agnostic root types (`AudioBackend` trait, `AudioError`, `SampleFormat`, lifecycle helpers) shared by every audio sibling crate.
- **NEW crate `blazen-audio-piper-vendored`** — fork of `piper-rs` patched to use `tract-onnx` (musl-clean) and subprocess `espeak-ng` instead of the upstream `onnxruntime` + bundled bindings. Documented in `docs/vendored-deps.md`.
- **Feature aliases on `blazen-llm`** (deprecated — removal in two release cycles): `whispercpp` → `audio-stt-whispercpp`, `piper` → `audio-tts-piper`, `tts` → `audio-tts-anytts`, `candle-audio` → `audio-music-musicgen`. Existing manifests continue to build through the transition window.
- **Umbrella `blazen/local-audio` feature** now activates the full new audio surface (TTS: anytts + openai + piper; STT: whispercpp + candle; music: musicgen + audiogen; codec: encodec + dac). Pre-restructure callers got `whispercpp` + `tts` + `candle-audio`; the new set is strictly broader.

### Added — PR-AUDIO restructure (Waves 1–28)

- **11 functional audio backends** spanning TTS / STT / music / codec, with one canonical `*Backend` trait per modality:
  - **TTS** — `AnyTtsBackend` (Kokoro-82M / VibeVoice / Qwen3-TTS via `any-tts`), `OpenAiTtsBackend` (HTTP client for `/v1/audio/speech` + `/v1/voices/*` voice-management extensions), `PiperBackend` (tract-onnx + subprocess `espeak-ng`).
  - **STT** — `WhisperCppBackend` (whisper.cpp via `whisper-rs`), `CandleWhisperBackend` (pure-Rust Whisper: 6 model variants Tiny/Base/Small/Medium/LargeV3 plus translate-to-English task).
  - **Music / SFX** — `MusicgenBackend` (real end-to-end MusicGen autoregressive port: T5 prompt encoder + decoder LM with delay-pattern interleaver + classifier-free guidance + EnCodec decoder; Small / Medium / Large variants), `AudioGenBackend` (reuses MusicGen infra against `facebook/audiogen-medium`).
  - **Codec** — `EncodecBackend` (Meta EnCodec 24 kHz, 4 codebooks via `candle-transformers`), `DacBackend` (Descript Audio Codec `descript/dac_44khz`, 9 codebooks @ 8 kbps — decode functional, encode upstream-gapped), `SnacBackend` (Multi-Scale Neural Audio Codec `hubertsiuzdak/snac_24khz`, 3 multi-scale codebooks at vq_strides `[4, 2, 1]` @ 24 kHz ≈ 3 kbps — encode + decode both functional).
- **Real MusicGen autoregressive head** — closes the prior MusicGen-as-stub limitation. Sampler with top-k / top-p / temperature, delay-pattern build / unbuild, hard duration safety cap (60 s).
- **Real AudioGen** — first-class text-to-SFX via the MusicGen autoregressive pipeline with AudioGen-specific 16 kHz EnCodec + 1.5B decoder LM weights.
- **Real candle Whisper STT** — 6 model variants from the Hugging Face hub (`openai/whisper-tiny` through `openai/whisper-large-v3`), mel filter bank loaded from `mel_filters_80.npz`, transcribe + translate-to-English tasks, optional language hint.
- **Real DAC + SNAC codec wrappers** — both via `candle-transformers`; SNAC loads via `VarBuilder::from_pth` (no safetensors mirror upstream).
- **Real Piper backend** — built on the new `blazen-audio-piper-vendored` crate. Subprocess `espeak-ng` for phonemization, `tract-onnx` for inference, no native `onnxruntime` dependency.
- **OpenAI HTTP TTS client** — `OpenAiTtsBackend` + `OpenAiTtsConfig` + `OpenAiTtsSpeechRequest` / `OpenAiTtsSpeechResponse` types. Speaks `/v1/audio/speech` (OpenAI standard) plus the de-facto `/v1/voices/*` extensions for voice listing / cloning / deletion.
- **Compatibility shim `blazen_llm::compat::whisper::WhisperCppProvider`** — preserves the pre-restructure `Arc<WhisperCppProvider>` call shape (`from_options`, `ComputeProvider`, `Transcription`, `LocalModel` impls). Bindings code (`blazen-py`, `blazen-node`, `blazen-uniffi`) keeps compiling untouched.
- **Documentation** — `docs/UNSUPPORTED_AUDIO.md` enumerates the 9 backends still gapped upstream (Stable Audio, Cosyvoice, Spark-TTS, …) with rationale per entry, and `docs/vendored-deps.md` records the Piper fork's diff.

### Added

- **`CustomProvider` exposed as a foreign-implementable typed trait in all 7 bindings** (py / node / wasm / go / swift / kotlin / ruby). Users subclass and override any of 16 typed methods (`complete`, `stream`, `embed`, plus 13 compute methods: `text_to_speech`, `generate_music`, `generate_sfx`, `clone_voice`, `list_voices`, `delete_voice`, `generate_image`, `upscale_image`, `text_to_video`, `image_to_video`, `transcribe`, `generate_3d`, `remove_background`). Unoverridden methods raise `Unsupported`. Replaces the prior JSON-string `HostDispatch` design (never released).
- **`ApiProtocol`, `BaseProvider`, `BaseProviderDefaults`, `CompletionProviderDefaults`, `EmbeddingProviderDefaults`** + **9 role-specific `*ProviderDefaults` types** (`AudioSpeechProviderDefaults`, `AudioMusicProviderDefaults`, `VoiceCloningProviderDefaults`, `ImageGenerationProviderDefaults`, `ImageUpscaleProviderDefaults`, `VideoProviderDefaults`, `TranscriptionProviderDefaults`, `ThreeDProviderDefaults`, `BackgroundRemovalProviderDefaults`) exposed in all 7 bindings.
- **15 compute request / response types** (`SpeechRequest`, `MusicRequest`, `ImageRequest`, `UpscaleRequest`, `VideoRequest`, `TranscriptionRequest`, `ThreeDRequest`, `VoiceCloneRequest`, `BackgroundRemovalRequest`, `AudioResult`, `ImageResult`, `VideoResult`, `TranscriptionResult`, `ThreeDResult`, `VoiceHandle`) exposed in all 7 bindings (previously py + node only).
- **Typed `extract<T>(schema, messages)` ergonomic API on `BaseProvider`** in every binding: pydantic `BaseModel` (Python), zod / JSON Schema (Node), JSON Schema (WASM), JSON Schema bytes (Go), `Decodable` (Swift), `KSerializer<T>` (Kotlin), Hash (Ruby).
- **Instance-level provider defaults** — system prompt, tools, response format, universal before-request hook, completion-specific hook, plus 9 role-specific defaults each with their own before-hook — configurable per provider instance through builder methods on `BaseProvider` and `CustomProviderHandle`.
- **Free factory functions** `ollama(host, port, model)`, `lm_studio(host, port, model)`, `openai_compat(provider_id, config)` in `blazen-llm` (also exposed as classmethods/module functions in every binding).
- `scripts/regen-bindings.sh` now syncs the Swift FFI header to `bindings/swift/Sources/BlazenFFI/include/BlazenFFI.h` after each Swift regen, eliminating the silent-staleness gap that previously required a manual `cp` step after UniFFI surface changes.
- WASM-SDK reaches feature parity with Node bindings: pipeline `input_mapper` / `condition` / `onPersist` / `onPersistJson` callbacks, workflow `setSessionPausePolicy` / `runStreaming` / `runWithHandler` / `resumeWithSerializableRefs`, handler `respondToInput` / `snapshot` / `resumeInPlace` / `streamEvents` / `abort`, context `insertSessionRefSerializable` / `getSessionRefSerializable`, real `ModelManager` bind, `InMemoryBackend` + `MemoryResult` standalone classes.
- WASM-SDK `TractEmbedModel.create(modelUrl, tokenizerUrl)` re-enabled via `web_sys::fetch`-based model + tokenizer loader (no `hf-hub` dep on wasm32).
- WASM-SDK `OtlpConfig` + `initOtlp` via custom `WasmFetchHttpClient` HTTP transport (works around `opentelemetry-otlp/grpc-tonic` incompatibility with wasm32).
- `MediaSource` type alias of `ImageSource` exposed in all three bindings.
- Backend inference types bound to py + node: 9 mistralrs (`ChatMessageInput`, `ChatRole`, `InferenceChunk`, `InferenceChunkStream`, `InferenceImage`, `InferenceImageSource`, `InferenceResult`, `InferenceToolCall`, `InferenceUsage`), 6 llamacpp (under `LlamaCpp` prefix), 1 candle (`CandleInferenceResult`).
- `LangfuseConfig` + `init_langfuse` exporter on py + node (new `langfuse` feature in `blazen-telemetry`).
- Python: `OtlpConfig` + `init_otlp` and `init_prometheus(port)` now exposed in `blazen.pyi`.
- Python error hierarchy now includes 9 per-backend `ProviderError` subclasses (`LlamaCppError`, `CandleLlmError`, `CandleEmbedError`, `MistralRsError`, `WhisperError`, `PiperError`, `DiffusionError`, `FastEmbedError`, `TractError`).
- Node binding: typed JS error class hierarchy (~87 classes via JS shim post-processor, rooted at `BlazenError`), `AgentResult` / `BatchResult` upgraded from plain dicts to typed classes with getters and `toString()`, `PipelineBuilder.onPersist` / `.onPersistJson`, `ProgressCallback` subclassable class, `enrichError(err)` helper.
- Python binding: `ProgressCallback` ABC.
- `blazen-telemetry` gained `otlp-http` Cargo feature for HTTP transport (alongside the existing `otlp` gRPC variant).
- WASM-SDK: `ModelRegistry` JS-callback ABC (`new ModelRegistry({ listModels, getModel })`) — parity with Python `PyModelRegistry` / Node `JsModelRegistry`. Auto-emitted `ModelRegistryImpl` interface in the regenerated `blazen_wasm_sdk.d.ts`.

### Changed

- CI's `audit-bindings` job now regenerates all three typegens (`blazen.pyi`, `index.d.ts`, `pkg/blazen_wasm_sdk.d.ts`) and fails on drift before running the parity audit.
- `cargo run --example stub_gen -p blazen-py` now requires `--features langfuse,otlp,prometheus,tract,distributed` to surface all feature-gated bindings (CI, `.githooks/pre-commit`, and `CLAUDE.md` updated).
- `auto-tag` release job now blocked on `audit-bindings`.
- **Breaking** — Node `ModelManager` byte-budget API now uses `bigint` instead of `number`: `ModelManagerConfig.budgetBytes`, `JsModelStatus.vramEstimate`, `register(..., vramEstimateBytes)`, `usedBytes()`, `availableBytes()`. Fixes silent truncation of >4 GiB budgets that affected typical 7B+ local LLMs. The `budgetGb: number` constructor path is unaffected.

### Fixed

- `blazen-node`: `JsAgentResult.toString()` and `JsBatchResult.toString()` no longer trip clippy's `inherent_to_string` lint (renamed Rust method, kept JS-facing `toString` via napi `js_name`).
- `blazen-py`: 9 doctest failures fixed by converting `Example::` indented Python blocks and `>>>` REPL examples in `prompts.rs`, `tool.rs`, `transcription.rs`, `providers/completion_model.rs`, and `workflow/step.rs` docstrings to explicit ` ```text ` fenced blocks (rustdoc was trying to compile Python as Rust). Same docstrings now appear in the regenerated `blazen.pyi` with cleaner formatting.
- Workspace `cargo clippy --workspace --all-features --tests -- -D warnings` is now clean (was failing on 22 pre-existing test-only lints across `blazen-llm-candle`, `blazen-memory-valkey`, `blazen-memory`, `blazen-embed-fastembed`, `blazen-embed-tract`, `blazen-llm-llamacpp`, `blazen-llm`, `blazen-pipeline`).

### CI

- **`aarch64-unknown-linux-musl` wheels and napi binary now build.** The
  `rust-musl-cross:aarch64-musl` image ships only long-triple binaries
  (`aarch64-unknown-linux-musl-gcc`), but Rust's target spec defaults the
  linker to the short form (`aarch64-linux-musl-gcc`). Prior attempts to
  override this via `CARGO_TARGET_*_LINKER` in `$GITHUB_ENV` did not
  propagate through `pnpm` / `uvx` to the cargo subprocess on the Forgejo
  runner. The probe step now creates short-triple symlinks in both
  `/usr/local/musl/bin` and `/usr/local/bin` so cargo's default lookup
  succeeds with zero env-var plumbing.
- **`x86_64-pc-windows-msvc` wheels and napi binary now build.**
  `llama-cpp-sys-2`'s `build.rs` unconditionally forwards every `CMAKE_*`
  environment variable as a `-D` flag to cmake. The Windows runner host
  injects `CMAKE_C_COMPILER_LAUNCHER=ccache` / `CMAKE_CXX_COMPILER_LAUNCHER=ccache`
  into the process env from some machine- or user-scope source outside the
  workflow, which then reached cmake, which then made Ninja try to spawn
  `ccache` — failing with `CreateProcess: The system cannot find the file
  specified`. The `Build wheels` and `Build napi binary` steps are now
  split into Windows (PowerShell) and non-Windows (bash) siblings; the
  Windows branch does `Remove-Item Env:CMAKE_*_COMPILER_LAUNCHER` before
  spawning `uvx maturin build` / `pnpm exec napi build`, so child
  processes inherit a clean env and the `-D` flag is never emitted.
- Added `Diagnose inherited env (Windows)` step (permanent) that dumps
  `CMAKE_*` / `*LAUNCHER*` variables at Process, Machine, and User scope
  on every Windows build, so the source of any future launcher leak is
  immediately visible in the log.
