...
/// ```
///
/// This generates an `impl zagents_events::Event for AnalyzeEvent` with:
...
pub fn derive_event(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics zagents_events::Event for #name #ty_generics #where_clause {
            fn event_type() -> &'static str {
                concat!(module_path!(), "::", stringify!(#name))
            }

            fn event_type_id(&self) -> &'static str {
                <Self as zagents_events::Event>::event_type()
            }
            ...
            fn clone_boxed(&self) -> Box<dyn zagents_events::AnyEvent> {
                Box::new(self.clone())
            }
...
/// `zagents_core::StepRegistration` that wraps the original function
...
fn step_impl(step_attr: StepAttr, input_fn: ItemFn) -> syn::Result<TokenStream> {
    ...
    let emits_tokens = if !step_attr.emits.is_empty() {
        ...
        quote! {
            vec![#( <#emit_types as zagents_events::Event>::event_type() ),*]
        }
        ...
        quote! { vec![<#ok_type as zagents_events::Event>::event_type()] }
    };
    ...
    let handler_body = if returns_step_output {
        ...
        quote! {
            let typed_event = event
                ...
                .ok_or_else(|| zagents_core::WorkflowError::EventDowncastFailed {
                    expected: <#event_type as zagents_events::Event>::event_type(),
        ...
        quote! {
            ...
            Ok(zagents_core::StepOutput::Single(Box::new(result)))
        }
    };
    ...
    let expanded = quote! {
        ...
        fn #registration_fn_name() -> zagents_core::StepRegistration {
            zagents_core::StepRegistration {
                name: #fn_name_str.to_string(),
                accepts: vec![<#event_type as zagents_events::Event>::event_type()],
                emits: #emits_tokens,
                handler: ::std::sync::Arc::new(
                    |event: Box<dyn zagents_events::AnyEvent>,
