...
impl TrainableQwen2 {
    ...
    pub fn load_with_mode(
        ...
        // Qlora (4-bit quantized base + LoRA) is wired for Llama only in
        // PR-Q phase 1; Qwen2's quantized variant lands in phase 2 once
        // the Llama path proves the pattern.
        let targets: HashSet<String> = match mode {
            ...
            TrainMode::Qlora { .. } => {
                return Err(candle_core::Error::Msg(
                    "QLoRA on Qwen2 not yet implemented (PR-Q phase 1 ships Llama only; \
