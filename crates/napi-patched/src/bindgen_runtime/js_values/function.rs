...
impl<Args: JsValuesTupleIntoVec, Return: FromNapiValue> Function<'_, Args, Return> {
  ...
  pub fn call(&self, args: Args) -> Result<Return> {
    ...
    check_status!(
      // Use `null` (not `undefined`) for `this` to match Node's NAPI convention
      // and avoid `Illegal invocation` brand-check failures on workerd/V8 when
      // the JS callback ends up invoking a host-bound method.
      unsafe { sys::napi_get_null(self.env, &mut raw_this) },
