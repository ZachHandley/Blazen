...
#[cfg(not(feature = "noop"))]
type AsyncExecutorFn = std::sync::Arc<
...
#[cfg(not(feature = "noop"))]
static ASYNC_EXECUTOR: OnceLock<AsyncExecutorFn> = OnceLock::new();

#[cfg(not(feature = "noop"))]
/// Register a custom executor for `#[napi] async fn` futures.
///
/// On wasm32-wasi targets without `tokio_unstable`, napi-rs normally drives
/// async futures via `std::thread::spawn(|| block_on(inner))`. On embedders
/// that don't implement the WASI threads ABI (e.g. Cloudflare workerd),
/// this traps. Embedders can register a JS-microtask-driven executor here
/// to bypass the std::thread::spawn path entirely.
///
/// First writer wins; subsequent calls are no-ops (`Err` returned).
///
/// ### Example (Cloudflare Workers via napi-rs wasi binary)
/// ```ignore
/// napi::set_async_executor(Arc::new(|fut| my_microtask_scheduler::spawn(fut)));
/// ```
pub fn set_async_executor(f: AsyncExecutorFn) -> std::result::Result<(), AsyncExecutorFn> {
  ASYNC_EXECUTOR.set(f)
}
