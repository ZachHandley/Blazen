//! Python wrapper for [`WorkflowHandler`](zagents_core::WorkflowHandler).
//!
...
pub struct PyWorkflowHandler {
    ...
    inner: Arc<Mutex<Option<zagents_core::WorkflowHandler>>>,
}

impl PyWorkflowHandler {
    /// Create a new handler wrapping a Rust `WorkflowHandler`.
    pub fn new(handler: zagents_core::WorkflowHandler) -> Self {
        Self {
...
type PinnedEventStream = std::pin::Pin<
    Box<dyn tokio_stream::Stream<Item = Box<dyn zagents_events::AnyEvent>> + Send + Unpin>,
>;
