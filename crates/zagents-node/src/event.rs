...
///
/// Special handling:
/// - `"zagents::StartEvent"` -> [`zagents_events::StartEvent`]
/// - `"zagents::StopEvent"` -> [`zagents_events::StopEvent`]
...
pub fn js_value_to_any_event(value: &serde_json::Value) -> Box<dyn AnyEvent> {
    ...
    if event_type == "zagents::StartEvent" {
        return Box::new(zagents_events::StartEvent { data });
    }
...
pub fn any_event_to_js_value(event: &dyn AnyEvent) -> serde_json::Value {
    ...
    // StopEvent: { type: "zagents::StopEvent", result: ... }
    if event_type == "zagents::StopEvent" {
        let result = json
            .get("result")
            .cloned()
...
mod tests {
    ...
    fn start_event_roundtrip() {
        let js = serde_json::json!({
            "type": "zagents::StartEvent",
            "message": "hello"
        });
        let event = js_value_to_any_event(&js);
        assert_eq!(event.event_type_id(), "zagents::StartEvent");

        let back = any_event_to_js_value(&*event);
        assert_eq!(back["type"], "zagents::StartEvent");
    ...
    fn stop_event_roundtrip() {
        let js = serde_json::json!({
            "type": "zagents::StopEvent",
            "result": {"answer": 42}
        });
        let event = js_value_to_any_event(&js);
        assert_eq!(event.event_type_id(), "zagents::StopEvent");

        let back = any_event_to_js_value(&*event);
        assert_eq!(back["type"], "zagents::StopEvent");
