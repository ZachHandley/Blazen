//! Python wrapper for the OpenRouter provider.
...
#[gen_stub_pyclass]
#[pyclass(name = "OpenRouterProvider", from_py_object)]
#[derive(Clone)]
pub struct PyOpenRouterProvider {
    inner: Arc<OpenRouterProvider>,
}

#[gen_stub_pymethods]
#[pymethods]
impl PyOpenRouterProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(OpenRouterProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
