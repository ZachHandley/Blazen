//! Python wrapper for the Fireworks AI provider.
...
#[gen_stub_pyclass]
#[pyclass(name = "FireworksProvider", from_py_object)]
#[derive(Clone)]
pub struct PyFireworksProvider {
...
impl PyFireworksProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(
                FireworksProvider::from_options(opts.clone()).map_err(blazen_error_to_pyerr)?,
            ),
            options: opts,
        })
    }
    ...
    fn embedding_model(&self) -> PyResult<PyEmbeddingModel> {
        let model =
            OpenAiCompatEmbeddingModel::embedding_from_options("fireworks", self.options.clone())
                .map_err(blazen_error_to_pyerr)?;
