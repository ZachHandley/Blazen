//! Python wrapper for the xAI (Grok) provider.
...
#[gen_stub_pyclass]
#[pyclass(name = "XaiProvider", from_py_object)]
#[derive(Clone)]
pub struct PyXaiProvider {
    inner: Arc<XaiProvider>,
}
...
impl PyXaiProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(XaiProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
