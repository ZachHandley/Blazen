//! Python wrapper for the Perplexity provider.
...
#[gen_stub_pyclass]
#[pyclass(name = "PerplexityProvider", from_py_object)]
#[derive(Clone)]
pub struct PyPerplexityProvider {
    inner: Arc<PerplexityProvider>,
}

#[gen_stub_pymethods]
#[pymethods]
impl PyPerplexityProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(PerplexityProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
