//! Python wrapper for the Groq provider (ultra-fast LPU inference).
...
#[gen_stub_pyclass]
#[pyclass(name = "GroqProvider", from_py_object)]
#[derive(Clone)]
pub struct PyGroqProvider {
    inner: Arc<GroqProvider>,
}
...
impl PyGroqProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(GroqProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
