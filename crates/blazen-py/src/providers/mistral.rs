//! Python wrapper for the Mistral AI provider.
...
#[gen_stub_pyclass]
#[pyclass(name = "MistralProvider", from_py_object)]
#[derive(Clone)]
pub struct PyMistralProvider {
    inner: Arc<MistralProvider>,
}

#[gen_stub_pymethods]
#[pymethods]
impl PyMistralProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(MistralProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
