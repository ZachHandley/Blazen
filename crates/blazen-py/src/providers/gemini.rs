//! Python wrapper for the Google Gemini provider.
//!
//! Exposes [`GeminiProvider`](blazen_llm::providers::gemini::GeminiProvider)
...
#[derive(Clone)]
pub struct PyGeminiProvider {
    inner: Arc<GeminiProvider>,
}

#[gen_stub_pymethods]
#[pymethods]
impl PyGeminiProvider {
    ...
    fn new(options: Option<PyRef<'_, PyProviderOptions>>) -> PyResult<Self> {
        ...
        Ok(Self {
            inner: Arc::new(GeminiProvider::from_options(opts).map_err(blazen_error_to_pyerr)?),
        })
    }
