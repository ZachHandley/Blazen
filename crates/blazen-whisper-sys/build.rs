...
fn main() {
    ...
    // `system-ggml` cargo feature is on. The upstream whisper.cpp cmake
    // already exposes WHISPER_USE_SYSTEM_GGML which makes its build call
    // `find_package(ggml REQUIRED)` instead of compiling its in-tree
    // ggml. We point that find_package at the blazen-ggml-sys install
    // prefix via CMAKE_PREFIX_PATH (DEP_GGML_PREFIX is set by cargo because
    // blazen-ggml-sys declares `links = "ggml"`).
    #[cfg(feature = "system-ggml")]
    {
        let ggml_prefix = env::var("DEP_GGML_PREFIX")
            .expect("DEP_GGML_PREFIX must be set when `system-ggml` feature is on (provided by blazen-ggml-sys via its `links = \"ggml\"` Cargo manifest entry)");
        println!("cargo:warning=blazen-whisper-sys: routing ggml via blazen-ggml-sys at {ggml_prefix}");
    ...
    // When the `system-ggml` Blazen fork feature is on, ggml comes from
    // blazen-ggml-sys (which already emitted its own
    // `cargo:rustc-link-lib=static=ggml{,-base,-cpu}` lines from its
    // build.rs). Re-emitting them here would either no-op or, worse,
    // produce duplicate-symbol errors at link time — exactly the failure
    // mode this fork is meant to eliminate. So skip them in that mode.
    if cfg!(feature = "intel-sycl") {
