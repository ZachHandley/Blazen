...
fn main() {
    ...
    if cfg!(feature = "system-ggml") {
        ...
        // Blazen fork: point find_package(ggml) at blazen-ggml-sys's
        // install tree. DEP_GGML_PREFIX is set by cargo because we
        // declare blazen-ggml-sys as a dep + build-dep in Cargo.toml,
        // and blazen-ggml-sys's package has `links = "ggml"` so its
        // `cargo:prefix=...` metadata gets surfaced here as
        // DEP_GGML_PREFIX. Same wiring as the blazen-whisper-sys and
        // blazen-diffusion-sys forks.
        if let Ok(ggml_prefix) = std::env::var("DEP_GGML_PREFIX") {
            println!(
                "cargo:warning=blazen-llama-cpp-sys-2: routing ggml via blazen-ggml-sys at {ggml_prefix}"
            );
