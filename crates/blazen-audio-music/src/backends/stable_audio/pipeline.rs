...
impl StableAudioPipeline {
    ...
    pub async fn generate(
        ...
        let waveform_vec = tokio::task::spawn_blocking(move || -> Result<Vec<f32>, MusicError> {
        ...
        let wav_bytes = pcm_to_wav(&waveform_vec, sample_rate, STABLE_AUDIO_CHANNELS);
        let duration =
            waveform_vec.len() as f32 / (sample_rate as f32 * f32::from(STABLE_AUDIO_CHANNELS));
        Ok(GeneratedAudio {
            bytes: wav_bytes,
