...
// ---------------------------------------------------------------------------
// Embed options (manual mirror — not auto-generated by build.rs)
// ---------------------------------------------------------------------------
...
#[cfg(feature = "embed")]
impl From<JsEmbedOptions> for blazen_llm::EmbedOptions {
    fn from(val: JsEmbedOptions) -> Self {
        Self {
...
// ---------------------------------------------------------------------------
// Feature-gated embed factory (separate impl block required by napi-derive)
// ---------------------------------------------------------------------------
...
impl JsEmbeddingModel {
    ...
    /// const response = await model.embed(["Hello", "World"]);
    /// ```
    #[napi(factory, js_name = "embed")]
    pub fn embed_local(options: Option<JsEmbedOptions>) -> Result<Self> {
        let opts = options.map(Into::into).unwrap_or_default();
        let model = blazen_llm::EmbedModel::from_options(opts).map_err(to_napi_error)?;
        Ok(Self {
