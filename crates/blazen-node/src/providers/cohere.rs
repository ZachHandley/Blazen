//! JavaScript bindings for the Cohere provider.
...
#[napi(js_name = "CohereProvider")]
pub struct JsCohereProvider {
...
impl JsCohereProvider {
    ...
    pub fn create(options: Option<JsProviderOptions>) -> Result<Self> {
        ...
        Ok(Self {
            inner: Arc::new(CohereProvider::from_options(opts).map_err(blazen_error_to_napi)?),
        })
    }
    ...
    pub fn embedding_model(
        ...
        Ok(JsOpenAiCompatEmbeddingModel::wrap(Arc::new(
            OpenAiCompatEmbeddingModel::embedding_from_options("cohere", opts)
