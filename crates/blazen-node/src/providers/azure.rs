//! JavaScript bindings for the Azure `OpenAI` provider.
...
use crate::generated::JsAzureOptions;
...
/// ```
#[napi(js_name = "AzureOpenAiProvider")]
pub struct JsAzureOpenAiProvider {
...
impl JsAzureOpenAiProvider {
    /// Create a new Azure `OpenAI` provider.
    #[napi(factory)]
    pub fn create(options: JsAzureOptions) -> Result<Self> {
        Ok(Self {
            inner: Arc::new(
                AzureOpenAiProvider::from_options(options.into()).map_err(blazen_error_to_napi)?,
            ),
        })
    }
