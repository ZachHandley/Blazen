//! JavaScript bindings for the `DeepSeek` provider.
...
#[napi(js_name = "DeepSeekProvider")]
pub struct JsDeepSeekProvider {
...
impl JsDeepSeekProvider {
    ...
    pub fn create(options: Option<JsProviderOptions>) -> Result<Self> {
        ...
        Ok(Self {
            inner: Arc::new(DeepSeekProvider::from_options(opts).map_err(blazen_error_to_napi)?),
        })
    }
