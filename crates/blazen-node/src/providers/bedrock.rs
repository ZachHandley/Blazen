//! JavaScript bindings for the AWS Bedrock provider.
...
use crate::generated::JsBedrockOptions;
...
/// });
/// ```
#[napi(js_name = "BedrockProvider")]
pub struct JsBedrockProvider {
...
impl JsBedrockProvider {
    /// Create a new Bedrock provider.
    #[napi(factory)]
    pub fn create(options: JsBedrockOptions) -> Result<Self> {
        Ok(Self {
            inner: Arc::new(
                BedrockProvider::from_options(options.into()).map_err(blazen_error_to_napi)?,
            ),
        })
    }
