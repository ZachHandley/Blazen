//! JavaScript bindings for the `OpenRouter` provider.
...
#[napi(js_name = "OpenRouterProvider")]
pub struct JsOpenRouterProvider {
...
impl JsOpenRouterProvider {
    ...
    pub fn create(options: Option<JsProviderOptions>) -> Result<Self> {
        ...
        Ok(Self {
            inner: Arc::new(OpenRouterProvider::from_options(opts).map_err(blazen_error_to_napi)?),
        })
    }
