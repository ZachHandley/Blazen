//! JavaScript bindings for the Fireworks AI provider.
...
use crate::providers::embedding_models::JsOpenAiCompatEmbeddingModel;
use crate::types::{
...
#[napi(js_name = "FireworksProvider")]
pub struct JsFireworksProvider {
    inner: Arc<FireworksProvider>,
}
...
impl JsFireworksProvider {
    ...
    pub fn create(options: Option<JsProviderOptions>) -> Result<Self> {
        ...
        Ok(Self {
            inner: Arc::new(FireworksProvider::from_options(opts).map_err(blazen_error_to_napi)?),
        })
    }
    ...
    pub fn embedding_model(
        options: Option<JsProviderOptions>,
    ) -> Result<JsOpenAiCompatEmbeddingModel> {
        let opts: ProviderOptions = options.map(Into::into).unwrap_or_default();
        Ok(JsOpenAiCompatEmbeddingModel::wrap(Arc::new(
            OpenAiCompatEmbeddingModel::embedding_from_options("fireworks", opts)
