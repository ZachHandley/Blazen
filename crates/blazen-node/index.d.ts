 ...
 * - `"blazen::StartEvent"` -- emitted to kick off a workflow.
 * - `"blazen::StopEvent"` -- emitted to terminate a workflow with a result.
 */
export interface Event {
  /** The event type identifier (e.g. `"AnalyzeEvent"`, `"blazen::StartEvent"`). */
  type: string;
...
export interface WorkflowResult {
  /** The event type of the final event (typically `"blazen::StopEvent"`). */
  type: string;
  /** The result data. */
...
export class Workflow {
  ...
  /**
   * Run the workflow and return a handler for fine-grained control.
   *
   * Unlike `run()` which awaits the result internally, this returns a
   * `WorkflowHandler` that lets you pause the workflow, stream events,
   * or await the result at your own pace.
   *
   * @param input - The input data (wrapped in a StartEvent).
   * @returns A handler for the running workflow.
   *
   * @example
   * ```typescript
   * const handler = await workflow.runWithHandler({ message: "hello" });
   *
   * // Option A: pause and save snapshot
   * const snapshot = await handler.pause();
   * fs.writeFileSync("snapshot.json", snapshot);
   *
   * // Option B: stream events then get result
   * handler.streamEvents((event) => console.log(event));
   * const result = await handler.result();
   * ```
   */
  runWithHandler(input: Record<string, any>): Promise<WorkflowHandler>;

  /**
   * Resume a previously paused workflow from a snapshot.
   *
   * The snapshot JSON string should have been obtained from a prior
   * call to `handler.pause()`. The workflow resumes with the same
   * steps registered on this `Workflow` instance.
   *
   * @param snapshotJson - The snapshot JSON string from `handler.pause()`.
   * @returns A handler for the resumed workflow.
   *
   * @example
   * ```typescript
 ...
 *
 * Provides methods to await the result, pause the workflow, or subscribe
 * to intermediate events. Returned by `Workflow.runWithHandler()` or
 * `Workflow.resume()`.
 *
...
export class WorkflowHandler {
   ...
   * This consumes the handler -- it can only be called once, and
   * cannot be called after `pause()`.
   */
   ...
   *
   * The snapshot contains all workflow state (context, pending events,
   * metadata) and can be saved to a file or database. Use
   * `workflow.resume(snapshotJson)` to resume later.
   *
   * This consumes the handler -- it can only be called once, and
   * cannot be called after `result()`.
   */
   ...
   *
   * Must be called **before** `result()` or `pause()`. Events
