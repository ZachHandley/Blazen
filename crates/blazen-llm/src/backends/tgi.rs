//! Bridge between [`blazen_llm_tgi::TgiProvider`] and the Blazen
//! [`CompletionModel`](crate::CompletionModel) +
//! [`LocalModel`](crate::LocalModel) traits.
//!
//! TGI exposes both an OpenAI-compatible chat surface (TGI ≥ 1.4) and
//! its own native `/generate` / `/generate_stream` shape. The bridge
//! prefers the OAI chat shape on the wire because it carries
//! `messages` natively (no manual template assembly) and the streaming
//! wire format is `OpenAI` SSE — identical to the vLLM bridge.
//!
//! The crate dependency edge is one-way: `blazen-llm` depends on
//! `blazen-llm-tgi`, never the reverse. All trait impls live here.

use std::pin::Pin;

use async_trait::async_trait;
use blazen_llm_tgi::{ActiveAdapter, TgiAdapterTransport, TgiError, TgiProvider};
...
// Streaming delta shape (OAI chat SSE).
...
/// Translate a Blazen `CompletionRequest` into a TGI OpenAI-compat
/// chat body. Per-request adapter selection uses the `adapter_id`
/// field (TGI-specific), set from the request's `model` field when it
/// looks like an adapter id (i.e. differs from the provider default).
fn build_body(default_model: &str, req: &CompletionRequest, stream: bool) -> Value {
    ...
    // If the request overrode `model` to something other than the base,
    // treat it as an adapter id. TGI uses `adapter_id` for routing; the
    // OAI `model` field is informational on this surface.
    if model != default_model {
        body["adapter_id"] = Value::String(model.to_string());
    }
...
fn parse_finish_reason(reason: &str) -> String {
    // `finish_reason` on both response and stream-chunk is `Option<String>`.
    // The bridge keeps the upstream wire string verbatim — downstream
    // consumers do the semantic mapping via the separate `FinishReason`
    // enum exported from `blazen-llm`.
    reason.to_string()
}

fn decode_response(value: Value) -> Result<CompletionResponse, BlazenError> {
    let parsed: WireResponse = serde_json::from_value(value)
        .map_err(|e| BlazenError::provider("tgi", format!("decode response: {e}")))?;
...
fn tgi_to_blazen(err: TgiError) -> BlazenError {
    match err {
        TgiError::Unsupported(msg) | TgiError::InvalidOptions(msg) => {
            BlazenError::unsupported(format!("tgi: {msg}"))
        }
        other => BlazenError::provider("tgi", other.to_string()),
    }
}

fn convert_transport(t: &AdapterTransport) -> TgiAdapterTransport {
    match t {
        AdapterTransport::LocalFs(p) => TgiAdapterTransport::LocalFs(p.clone()),
        AdapterTransport::HfHub { repo, revision } => TgiAdapterTransport::HfHub {
            repo: repo.clone(),
            revision: revision.clone(),
        },
        AdapterTransport::HttpPush(bytes) => TgiAdapterTransport::HttpPush(bytes.clone()),
    }
}

// ---------------------------------------------------------------------------
// SSE parser — minimal, scoped to the tgi bridge
// ---------------------------------------------------------------------------
...
#[async_trait]
impl CompletionModel for TgiProvider {
    fn model_id(&self) -> &str {
        TgiProvider::model_id(self)
    }
    ...
    async fn complete(
        ...
        let value = self.chat(body).await.map_err(tgi_to_blazen)?;
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<StreamChunk, BlazenError>> + Send>>, BlazenError>
    {
        ...
        let response = self.chat_stream(body).await.map_err(tgi_to_blazen)?;
...
#[async_trait]
impl crate::traits::LocalModel for TgiProvider {
    /// TGI owns its own model lifecycle — `load` verifies the upstream
    /// can reach the base model via `GET /v1/models`.
    async fn load(&self) -> Result<(), BlazenError> {
        self.refresh_adapters_from_server()
            .await
            .map(|_| ())
            .map_err(tgi_to_blazen)
    }

    /// TGI has no public unload API — this is a no-op success
    /// (idempotent contract).
    ...
    async fn load_adapter(
        ...
        // Convert the caller's transport for parity but the provider
        // already enforces the policy (LocalFs / HfHub accepted as
        // "preloaded id" markers; HttpPush rejected).
        let _ = convert_transport(&AdapterTransport::default());

        let active: ActiveAdapter = self
...
mod tests {
    ...
    fn build_body_uses_provider_default_model_when_request_omits_one() {
        ...
        // No adapter_id when caller did not override the model.
    ...
    #[test]
    fn build_body_treats_request_model_override_as_adapter_id() {
        let mut req = make_req("hi");
        req.model = Some("sql-lora".into());
        let body = build_body("llama-3.2", &req, true);
        // The OAI `model` stays as the base; `adapter_id` carries the override.
        assert_eq!(body["model"], "llama-3.2");
        assert_eq!(body["adapter_id"], "sql-lora");
    ...
    fn convert_transport_passes_through_each_variant() {
        ...
        assert!(matches!(lf, TgiAdapterTransport::LocalFs(_)));
        let hf = convert_transport(&AdapterTransport::HfHub {
            repo: "org/repo".into(),
            revision: Some("v1".into()),
        });
        assert!(matches!(hf, TgiAdapterTransport::HfHub { .. }));
        let push = convert_transport(&AdapterTransport::HttpPush(vec![1, 2]));
        assert!(matches!(push, TgiAdapterTransport::HttpPush(_)));
    }

    #[test]
    fn tgi_to_blazen_routes_unsupported_to_unsupported_variant() {
        let e = tgi_to_blazen(TgiError::Unsupported("nope".into()));
        assert!(format!("{e:?}").to_lowercase().contains("unsupported"));
    }

    #[test]
    fn tgi_to_blazen_routes_validation_to_provider_variant() {
        let e = tgi_to_blazen(TgiError::Validation("bad input".into()));
