...
// `baked_pricing()` is generated by build.rs from `data/pricing.json`.
include!(concat!(env!("OUT_DIR"), "/baked_pricing.rs"));

/// Default pricing entries seeded into the registry.
///
/// Source of truth is `crates/blazen-llm/data/pricing.json`, baked into the
/// binary by `build.rs`. The daily check-pricing workflow refreshes that JSON
/// (committing to the `models-data` branch and uploading to the `blazen.dev`
/// Cloudflare Worker); the release flow syncs `models-data` → `main` before
/// each release so published artifacts ship with the latest snapshot. Apps
/// wanting always-fresh pricing should call [`refresh_default`](crate::refresh_default)
/// at startup.
