...
#
# Blazen CI base image for aarch64-unknown-linux-gnu (manylinux_2_28).
#
# Pre-bakes everything `.forgejo/workflows/build-artifacts.yaml` installs at
# runtime for the aarch64 glibc / manylinux matrix slot, so Forgejo jobs skip
...
# Build:
#   podman build \
#     -f aarch64-unknown-linux-gnu.Dockerfile \
#     -t blazen-ci-base:aarch64-unknown-linux-gnu-local .
#
...
# the `aarch64-unknown-linux-musl` tarball here too: Mozilla's musl build is
...
RUN set -eux; \
    ...
    rustc --version; \
    cargo --version; \
    rustup target list --installed | grep -Fx aarch64-unknown-linux-gnu; \
    uv --version; \
    maturin --version
