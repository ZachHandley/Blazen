...
All providers accept an API key and an optional options object. If `model` is omitted, the provider's default model is used.
...
| `CompletionModel.fal` | `(apiKey: string, options?: ProviderOptions)` |
