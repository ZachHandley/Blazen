...
`new XProvider(options)` and call `complete()` on it. Each provider class wraps
the same Rust provider as the matching `Model.x()` shorthand, but the class form
is the preferred surface: it is explicit about which provider you are using, takes
an options object (`apiKey`, `model`, `baseUrl`) at construction time, and exposes
that provider's full capability set (e.g. `OpenAiProvider.textToSpeech`,
`FalProvider.generateImage`).

The provider-class constructor accepts a plain JS options object. When `apiKey`
is omitted it is read from the provider's standard environment variable
(`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `FAL_KEY`, …); when `model` is omitted the
...
// Reads OPENAI_API_KEY from the environment; default model.
const openai = new OpenAiProvider({});

// Explicit key + model override.
const openaiExplicit = new OpenAiProvider({ apiKey: "sk-...", model: "gpt-4o" });
...
Every LLM provider class exposes the same methods:
...
provider.stream(messages: ChatMessage[], callback: (chunk) => void): Promise<void>
provider.toModel(): Model   // convert to a generic Model for runAgent / decorators / ModelManager
```

**Provider classes**

Construct any of these directly with `new XProvider(options)`:

| Class | Constructor |
|---|---|
| `OpenAiProvider` | `new OpenAiProvider({ apiKey?, model?, baseUrl? })` |
| `AnthropicProvider` | `new AnthropicProvider({ apiKey?, model?, baseUrl? })` |
| `AzureOpenAiProvider` | `new AzureOpenAiProvider({ ... })` |
| `OpenRouterProvider` | `new OpenRouterProvider({ apiKey?, model?, baseUrl? })` |
| `BedrockProvider` | `new BedrockProvider({ ... })` |
| `FalProvider` | `new FalProvider({ apiKey?, model? })` |

Call `.toModel()` on any provider instance to obtain a generic `Model` (for use
with `runAgent`, decorators, `Model.withFallback`, or registering in a
[`ModelManager`](#modelmanager)). The `OpenAiProvider` and `FalProvider` classes
also carry extra capabilities beyond chat — `OpenAiProvider.textToSpeech(...)` and
the full `FalProvider` image/video/audio/3D surface. See
[media generation](/guides/media-generation/) for those.
...
[`ModelManager`](#modelmanager)). For single-provider code, prefer the
[direct provider class](#constructing-a-provider--direct).
...
| `Model.anthropic` | `()` |
| `Model.gemini` | `()` |
| `Model.azure` | `(resourceName: string, deploymentName: string)` |
| `Model.fal` | `()` |
| `Model.openrouter` | `()` |
| `Model.groq` | `()` |
| `Model.together` | `()` |
...
| `Model.xai` | `()` |
| `Model.cohere` | `()` |
| `Model.bedrock` | `(region: string)` |

```typescript
const model = Model.openai();
const claude = Model.anthropic();
const gemini = Model.gemini();
const azure = Model.azure('my-resource', 'my-deployment');
const fal = Model.fal();
const groq = Model.groq().withModel('llama-3.3-70b-versatile');
const bedrock = Model.bedrock('us-east-1');
```

#### `model.withModel(modelId: string): Model`

Override the default model ID for this provider instance. Returns a new `Model` (WASM does not mutate in place).

```typescript
const model = Model.openai().withModel('gpt-4o-mini');
```
...
Each decorator returns a new `Model` wrapping the original.

#### `model.withRetry(maxRetries?: number): Model`
...
#### `model.withCache(ttlSeconds?: number, maxEntries?: number): Model`
...
`Model` and `EmbeddingModel` can be created from JavaScript handler functions using static factory methods. This lets you implement custom providers without subclassing.

### Model.fromJsHandler

```typescript
const model = Model.fromJsHandler("my-llm", async (request) => {
...
The handler receives a request object and should return a `ModelResponse`-shaped object.
...
A `ModelManager` is a **named registry for providers** — remote *and* local — that
...
```typescript
import init, { ModelManager, Model, ChatMessage } from "@blazen/sdk";

await init();

const manager = new ModelManager(0); // no memory budget needed for remote-only

// Remote providers register as dispatch-only entries (memoryEstimateBytes = 0,
// no lifecycle object).
await manager.register("fast", Model.openai(), 0);
await manager.register("smart", Model.anthropic(), 0);
...
The same registry is also a **per-pool VRAM/RAM budgeter for local weights**: pass
a `lifecycle` object (with async `load()` / `unload()`, plus optional `isLoaded()`,
`memoryBytes()`, `device()`) when registering a browser-side resource (a WebLLM /
WebGPU engine you load yourself, or a BYO backend via `registerByoBackend`). Such
entries participate in load/unload and per-pool LRU eviction; `memoryEstimateBytes`
reports their footprint. Remote providers own no local weights, so they never count
against a budget.
...
| Method | Signature | Description |
|---|---|---|
| `register` | `await manager.register(id, model: Model \| null, memoryEstimateBytes: number, lifecycle?)` | Register a provider under `id`. Pass a `Model` (e.g. `Model.openai()`) for a remote dispatch-only entry — use `memoryEstimateBytes = 0` and omit `lifecycle`; it never counts against a budget. For a lifecycle-only browser resource, pass `model = null` and supply a `lifecycle` object (async `load()` / `unload()`, optional `isLoaded()`, `memoryBytes()`, `device()`). Pass both to get a dispatchable *and* budgeted entry. |
| `complete` | `await manager.complete(id, messages): Promise<ModelResponse>` | Run a chat completion against the provider registered under `id`. Local entries auto-load on first use; remote entries dispatch straight through. Rejects if `id` is unknown or was registered for lifecycle only. |
| `stream` | `await manager.stream(id, messages, onChunk)` | Streaming counterpart to `complete`. `onChunk` is invoked once per `StreamChunk`. |
| `get` | `await manager.get(id): Promise<Model \| null>` | Fetch the chat provider registered under `id` to use or compose directly (e.g. wrap several in a fallback). Resolves to `null` if `id` is unknown or was registered for lifecycle only. |
| `registerByoBackend` | `await manager.registerByoBackend(id, backend)` | Register a Bring-Your-Own JS inference backend (object with `complete(req)`, optional `streamComplete(req, onChunk)`, `memoryBytes`, `device`). Dispatchable and budgeted via its own `memoryBytes`. |
