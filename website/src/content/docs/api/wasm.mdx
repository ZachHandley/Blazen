...
> **Backed by the real `blazen_manager::ModelManager`.** Earlier WASM builds shipped a `HashMap` reimpl with the same surface; the binding now wraps the real Rust manager, so eviction order, budget accounting, and concurrent-access semantics match the native and Node bindings exactly. The JS API surface (`register`, `unregister`, `load`, `unload`, `isLoaded`, `usedBytes`, `availableBytes`, `budgetBytes`, `status`) is unchanged.
