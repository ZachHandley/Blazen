...
| `candle-llm` | `CandleLlmProvider`, `CandleLlmModel`, `CandleInferenceResult`, `CandleLlmError`, `CandleLlmOptions` |
...
| Feature | Description |
|---------|-------------|
| `spans` (default) | Enables `TracingModel` and per-span instrumentation hooks |
...
Extract typed data from a model using JSON Schema constraints. This trait has a **blanket implementation** for every `Model` -- providers do not need to implement it.
...
// Blanket impl: every Model automatically gets this.
...
| `Handle { handle }` | Reference to a [`ContentHandle`](#contenthandle) registered with a [`ContentStore`](#contentstore). Resolved into a concrete `Url` / `Base64` / `ProviderFile` by [`ModelRequest::resolve_handles_with`](#completionrequestresolve_handles_with) before the request hits a provider. |
...
| `resolve` | Produce the concrete [`MediaSource`](#imagesource) the provider will see (typically `Url`, `Base64`, or `ProviderFile`). Called by [`ModelRequest::resolve_handles_with`](#completionrequestresolve_handles_with). |
...
| `prepare_request_with_store` | One-call convenience: runs [`ModelRequest::resolve_handles_with`](#completionrequestresolve_handles_with), then builds and prepends the system note. Returns the number of resolved handles. This is what the agent loop calls before dispatching a request. |
...
### `ModelRequest::resolve_handles_with`
...
1. Build a `ModelRequest` with the full message history and all tool definitions.
2. Call the model.
...
| Field | Type | Description |
|-------|------|-------------|
| `response` | `ModelResponse` | The final completion response |
...
A type can implement both `Model` and `LocalModel`:

```rust
struct MyLocalLLM { /* ... */ }

#[async_trait::async_trait]
impl Model for MyLocalLLM {
    ...
    async fn stream(&self, request: ModelRequest) -> Result<Pin<Box<dyn Stream<Item = Result<StreamChunk, BlazenError>> + Send>>, BlazenError> {
        todo!()
    }
}
...
| `Model` | `ModelErrorKind` | LLM model-call error |
...
```rust
use blazen_llm::{
    Model, ModelRequest, ModelResponse, StreamChunk, BlazenError,
};
use std::pin::Pin;
use futures_util::Stream;
...
|----------|---------|-------------------|
| `OpenAiProvider` | `openai` | `Model`, `StructuredOutput` |
| `OpenAiCompatProvider` | `openai` | `Model`, `StructuredOutput`, `ModelRegistry` |
| `AnthropicProvider` | `anthropic` | `Model`, `StructuredOutput` |
| `GeminiProvider` | `gemini` | `Model`, `StructuredOutput`, `ModelRegistry` |
| `AzureOpenAiProvider` | `azure` | `Model`, `StructuredOutput` |
| `FalProvider` | `fal` | `Model`, `StructuredOutput`, `ComputeProvider`, `ImageGeneration`, `VideoGeneration`, `AudioGeneration`, `Transcription` |
...
Candle exposes its result type as `CandleInferenceResult` (already prefixed upstream). The `CandleLlmModel` adapter wraps the raw provider in the `Model` trait:

```rust
#[cfg(feature = "candle-llm")]
use blazen_llm::{
    CandleLlmProvider, CandleLlmModel,
