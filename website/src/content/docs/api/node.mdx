...
Store a JSON-serializable value in the workflow context.

### `await ctx.get(key: string): any`

Retrieve a value from the workflow context. Returns `null` if not found.

### `await ctx.setBytes(key: string, buffer: Buffer): void`

Store raw binary data in the workflow context. No serialization requirement -- store any type by converting to bytes yourself (e.g., MessagePack, protobuf). Binary data persists through pause/resume/checkpoint.

### `await ctx.getBytes(key: string): Buffer | null`

Retrieve raw binary data from the workflow context. Returns `null` if not found.
