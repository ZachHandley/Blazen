...
All providers accept an API key and an optional options object. If `options` is omitted (or `model` is not set within it), the provider's default model is used.

| Method | Signature |
|---|---|
| `CompletionModel.openai` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.anthropic` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.gemini` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.azure` | `(apiKey: string, options: AzureOptions)` |
| `CompletionModel.fal` | `(apiKey: string, options?: FalOptions)` |
| `CompletionModel.openrouter` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.groq` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.together` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.mistral` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.deepseek` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.fireworks` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.perplexity` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.xai` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.cohere` | `(apiKey: string, options?: ProviderOptions)` |
| `CompletionModel.bedrock` | `(apiKey: string, options: BedrockOptions)` |

```typescript
const model = CompletionModel.openai("sk-...");
const claude = CompletionModel.anthropic("sk-ant-...", { model: "claude-sonnet-4-20250514" });
const gemini = CompletionModel.gemini("AIza...", { model: "gemini-2.5-flash" });
```
