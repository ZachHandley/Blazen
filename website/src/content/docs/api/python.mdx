...
The canonical way to talk to a provider is to construct its class directly and
call `complete()` (or `stream()`) on it. Each provider class wraps the same Rust
provider as the matching `Model.x()` shorthand, but the class form is the
preferred surface: it is explicit about which provider you are using and exposes
that provider's full capability set (e.g. `OpenAiProvider.text_to_speech`,
`FalProvider.generate_image`).

API keys are resolved from `ProviderOptions(api_key=...)` (or `FalOptions`,
`AzureOptions`, `BedrockOptions` for those providers) when passed explicitly, or
from the provider's standard environment variable (e.g. `OPENAI_API_KEY`,
`ANTHROPIC_API_KEY`, `FAL_KEY`) when `options` is omitted.

```python
from blazen import OpenAiProvider, ProviderOptions, ChatMessage, ModelOptions
...
# Sampling/tool/response-format options:
response = await openai.complete(
    [ChatMessage.user("Write a haiku about Rust.")],
    ModelOptions(temperature=0.7, max_tokens=100),
)
```

Every LLM provider class exposes the same two methods:
...
def stream(
    ...
    messages: list[ChatMessage],
    options: ModelOptions | None = None,
) -> CompletionStream: ...   # async iterator — use `async for`
```

`stream()` returns a `CompletionStream` you iterate with `async for`:

```python
async for chunk in openai.stream([ChatMessage.user("Tell me a story")]):
    if chunk.delta:
...
Construct any of these directly with `Provider(options=...)`:

| Class | Options type | Direct construction |
|---|---|---|
| `OpenAiProvider` | `ProviderOptions` | `OpenAiProvider(options=ProviderOptions(...))` |
| `AnthropicProvider` | `ProviderOptions` | `AnthropicProvider(options=ProviderOptions(...))` |
| `GeminiProvider` | `ProviderOptions` | `GeminiProvider(options=ProviderOptions(...))` |
| `AzureOpenAiProvider` | `AzureOptions` | `AzureOpenAiProvider(options=AzureOptions(...))` |
| `OpenRouterProvider` | `ProviderOptions` | `OpenRouterProvider(options=ProviderOptions(...))` |
| `GroqProvider` | `ProviderOptions` | `GroqProvider(options=ProviderOptions(...))` |
| `TogetherProvider` | `ProviderOptions` | `TogetherProvider(options=ProviderOptions(...))` |
| `MistralProvider` | `ProviderOptions` | `MistralProvider(options=ProviderOptions(...))` |
| `DeepSeekProvider` | `ProviderOptions` | `DeepSeekProvider(options=ProviderOptions(...))` |
| `FireworksProvider` | `ProviderOptions` | `FireworksProvider(options=ProviderOptions(...))` |
| `PerplexityProvider` | `ProviderOptions` | `PerplexityProvider(options=ProviderOptions(...))` |
| `XaiProvider` | `ProviderOptions` | `XaiProvider(options=ProviderOptions(...))` |
| `CohereProvider` | `ProviderOptions` | `CohereProvider(options=ProviderOptions(...))` |
| `BedrockProvider` | `BedrockOptions` | `BedrockProvider(options=BedrockOptions(...))` |
| `FalProvider` | `FalOptions` | `FalProvider(options=FalOptions(...))` |

The `OpenAiProvider` and `FalProvider` classes carry extra capabilities beyond
chat — `OpenAiProvider.text_to_speech(...)`, and the full `FalProvider`
image/video/audio/3D surface. See [media generation](/guides/media-generation/)
for those.

## Model

`Model` is the generic, provider-agnostic chat handle. Its static
`Model.x()` factories are **shorthand constructors** that build the same
underlying provider as the classes above; reach for them when you want a uniform
`Model` type (e.g. to mix providers in `Model.with_fallback([...])` or to register
remote providers in a [`ModelManager`](#modelmanager)). For single-provider code,
prefer the [direct provider class](#constructing-a-provider--direct).
...
# Pass an explicit key
model = Model.anthropic(options=ProviderOptions(api_key="sk-ant-..."))
...
Each returns a generic `Model`. Direct construction of the provider class is
preferred for single-provider code; use these when you want the uniform `Model`
type.

| Constructor | Signature |
|---|---|
| `openai` | `Model.openai(*, options: ProviderOptions = None)` |
| `anthropic` | `Model.anthropic(*, options: ProviderOptions = None)` |
| `gemini` | `Model.gemini(*, options: ProviderOptions = None)` |
| `azure` | `Model.azure(*, options: AzureOptions)` |
| `openrouter` | `Model.openrouter(*, options: ProviderOptions = None)` |
| `groq` | `Model.groq(*, options: ProviderOptions = None)` |
| `together` | `Model.together(*, options: ProviderOptions = None)` |
| `mistral` | `Model.mistral(*, options: ProviderOptions = None)` |
| `deepseek` | `Model.deepseek(*, options: ProviderOptions = None)` |
| `fireworks` | `Model.fireworks(*, options: ProviderOptions = None)` |
| `perplexity` | `Model.perplexity(*, options: ProviderOptions = None)` |
| `xai` | `Model.xai(*, options: ProviderOptions = None)` |
| `cohere` | `Model.cohere(*, options: ProviderOptions = None)` |
| `bedrock` | `Model.bedrock(*, options: BedrockOptions)` |
| `fal` | `Model.fal(*, options: FalOptions = None)` |
...
Returns a typed `ModelResponse` (see below). Also supports dict-style access for backwards compatibility: `response["content"]`.
...
| `with_fallback` | `Model.with_fallback(models: list[Model])` | Static method. Tries providers in order; falls back on transient errors. |
...
model = Model.openai().with_cache().with_retry()
...
Timing metadata attached to a `ModelResponse`. All fields are optional since not every provider reports timing data.
...
The agent system provides an agentic tool-execution loop on top of `Model`. Define tools with `ToolDef`, then call `run_agent` to let the model iteratively call tools until it produces a final answer.
...
|---|---|---|---|
| `model` | `Model` | required | The model to use for completions. |
...
| Property | Type | Description |
|---|---|---|
| `.response` | `ModelResponse` | The final completion response from the model. |
...
Generate vector embeddings from text. Created via static constructor methods, similar to `Model`. Keys are read from environment variables (`OPENAI_API_KEY`, etc.) when `options` is omitted, or can be passed explicitly via `ProviderOptions(api_key=...)`.
...
```python
from blazen import ModelManager, Model, FalProvider, ChatMessage
...
await manager.register("image", FalProvider())

# Dispatch by name from anywhere.
resp = await manager.complete("smart", [ChatMessage.user("Explain CRDTs.")])
print(resp.content)
...
await manager.stream("fast", [ChatMessage.user("Count to 5")], on_chunk)

# Fetch a registered provider to use or compose directly.
smart = await manager.get("smart")
fallback = Model.with_fallback([await manager.get("fast"), smart])
```

The same registry is also a **per-pool VRAM/RAM budgeter for local weights**: when
you register a *local* backend (`Model.mistralrs(...)`, a `LlamaCppProvider` /
`CandleLlmProvider`, or a Hugging Face repo via `load_from_hf`), the manager
tracks its memory footprint
per pool (host RAM, or per-GPU VRAM) and applies LRU eviction within a pool when a
load would exceed the budget. Remote providers own no local weights, so they never
count against a budget. Models in different pools never evict each other.

`ModelManager` is a **memory budget bookkeeper**, not a performance scheduler. It
answers "will this fit?" -- not "will this run fast?". Whether a 70B model loaded on
CPU is *useful* at 1-3 tok/s is a workload-choice question the manager intentionally
does not answer.
...
| Method | Signature | Description |
|---|---|---|
| `register` | `await manager.register(id, model, *, memory_estimate_bytes=None)` | Register a provider under `id`. Remote providers (`Model.openai()`, `FalProvider(...)`, …) register as dispatch-only entries and never count against a budget; local backends additionally participate in load/unload + per-pool LRU eviction, with `memory_estimate_bytes` reporting their footprint. Duck-typed objects with `load`/`unload` are registered for lifecycle bookkeeping only (not dispatchable). |
| `complete` | `await manager.complete(id, messages, *, options=None) -> ModelResponse` | Run a chat completion against the provider registered under `id`. Local entries auto-load on first use (subject to budget); remote entries dispatch straight through. Raises `ValueError` if `id` is unknown, or if it was registered for lifecycle only. |
| `stream` | `await manager.stream(id, messages, on_chunk, *, options=None)` | Stream a chat completion against `id`, invoking `on_chunk` once per `StreamChunk`. For `async for` iteration, fetch the provider with `get(id)` and stream on it directly. |
| `get` | `await manager.get(id) -> Model \| None` | Fetch the provider registered under `id` to use or compose directly. Returns `None` if `id` is unknown or was registered for lifecycle only. |
...
```python
model = Model.openai()  # Remote -- these methods are no-ops
# For a local model:
await model.load()
