...
## Custom Model (trait impl)

In Rust, custom providers are built by implementing the `Model` trait. The trait-impl is a first-class citizen -- it works with `run_agent`, `with_retry`, `with_cache`, and every other helper.
...
use blazen_llm::{
    BlazenError, ModelRequest, ModelResponse, StreamChunk,
    traits::Model,
    types::Role,
};
...
```sh
cargo run -p blazen --example custom_model
```
...
Register pricing for any model ID (your own model, a local finetune, a custom deployment). Every `ModelResponse` carries a `.cost` field computed from the registered rate.

```rust
use blazen_llm::{
    ChatMessage, PricingEntry,
    providers::openai_compat::{AuthMethod, OpenAiCompatConfig, OpenAiCompatProvider},
    register_pricing, traits::Model, types::ModelRequest,
};
