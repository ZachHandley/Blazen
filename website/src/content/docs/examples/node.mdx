...
> **Note:** `poolBudgets` values, `memoryEstimateBytes`, `usedBytes`, `availableBytes`, and the `memoryEstimateBytes` field on `ModelStatus` are JS `bigint`s (since the Rust side uses `u64` and `>4 GiB` budgets used to silently truncate). `cpuRamGb` / `gpuVramGb` are still regular `number`s. `pool` is a plain string (`"cpu"`, `"gpu:0"`, ...).

```typescript
import { Model, ModelManager } from "blazen";
