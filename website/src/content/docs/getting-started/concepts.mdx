...
Steps that call an LLM do so through a **provider** (OpenAI, Anthropic, fal.ai, local backends, …) — construct one and call `complete()`, or register many in a `ModelManager` and dispatch by name. See [Models & Providers](/getting-started/providers/) for the canonical pattern.
