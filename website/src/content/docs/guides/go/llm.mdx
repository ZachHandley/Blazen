...
- `NewAzureModel(apiKey, resourceName, deploymentName, apiVersion)` -- Azure OpenAI deployments are named per-resource.
- `NewBedrockModel(apiKey, region, model, baseURL)` -- AWS Bedrock is region-scoped.
- `NewOpenAICompatModel(providerName, baseURL, apiKey, model)` -- a generic OpenAI-compatible factory for Ollama, vLLM, LM Studio, LocalAI, and similar. All four arguments are required; pass a placeholder `apiKey` when the upstream does not require one.
...
`ModelManager` is the same unified registry exposed across every Blazen binding; see the cross-language [Local Inference](/guides/local-inference/) guide for the budgeting model and how it also holds remote providers by name in the other bindings.
...
- [Agent](/guides/go/agent/) -- LLM tool-call loop built on `Model`.
