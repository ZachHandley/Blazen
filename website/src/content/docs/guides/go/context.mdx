...
| `Model.Complete(ctx, req)` | Returns `ctx.Err()` on cancellation; provider request continues. |
...
| `Workflow.Close()` / `Model.Close()` / `EmbeddingModel.Close()` / `Agent.Close()` | Releases the underlying native handle. Idempotent and safe to call from multiple goroutines. |
