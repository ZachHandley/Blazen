...
| Embeddings | FastEmbed (ONNX Runtime) | `blazen-embed-fastembed` | `fastembed` |
...
model = EmbeddingModel.fastembed(options=FastEmbedOptions(
...
const model = EmbeddingModel.fastembed({
  ...
  cacheDir: "/tmp/fastembed",
});

const resp = await model.embed(["hello", "world"]);
```

FastEmbed runs through ONNX Runtime -- CPU-only, no GPU required, no Python ML runtime needed. Models cache locally after the first download.
...
embedder = EmbeddingModel.fastembed()
...
- **CPU-only workloads** -- FastEmbed, whisper.cpp (without `cuda`/`metal`/`coreml`), llama.cpp CPU builds, Piper. Use when the deployment target has no GPU or when latency is not critical.
...
Feature flags are selected at build time. The default Python wheels ship with `fastembed`, `whispercpp`, and `mistralrs` enabled; enable more by building from source with extra flags.

## See also

- [Embeddings](/guides/python/embeddings/) -- deeper dive on FastEmbed model variants
