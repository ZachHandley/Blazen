...
In the Python and Node bindings, the agent runner calls the equivalent code automatically when an agent has a content store wired in -- you do not need to invoke it by hand. Drop down to the Rust helpers when you are running a `ModelRequest` directly without going through the agent runner, or when you want to inspect the resolved request before dispatch.
