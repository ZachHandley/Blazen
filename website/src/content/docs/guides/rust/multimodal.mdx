...
If your `ModelRequest` contains messages with `ImageSource::Handle { .. }` content, call `resolve_handles_with` before sending it to a provider that does not understand handles natively:
...
let store = InMemoryContentStore::new();
let mut request = ModelRequest::new("gpt-4o");
...
let store = InMemoryContentStore::new();
let mut request = ModelRequest::new("claude-sonnet-4-5");
