...
Blazen provides a unified `EmbeddingModel` interface for generating vector embeddings across multiple providers. The API mirrors `Model`: create a model with a static constructor, then call `embed()`.
