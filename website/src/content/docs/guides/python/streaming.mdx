...
Use `ctx.write_event_to_stream()` inside any step to push an event to the external stream:

```python
@step
async def process(ctx: Context, ev: Event):
    for i in range(3):
        ctx.write_event_to_stream(Event("Progress", step=i, message=f"Processing step {i}"))
...
```python
wf = Workflow("stream-demo", [process])
