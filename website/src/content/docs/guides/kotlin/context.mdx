...
Every UniFFI handle — `Workflow`, `WorkflowBuilder`, `Model`, `EmbeddingModel`, `Agent`, `Pipeline`, `CheckpointStore` — implements `AutoCloseable`. Use Kotlin's `use { }` block to close them deterministically:
