...
```python
from blazen import OpenAiProvider, ProviderOptions, ChatMessage, complete_batch
...
```typescript
import { OpenAiProvider, ChatMessage, completeBatch } from "blazen";

const model = OpenAiProvider.create({ apiKey: process.env.OPENAI_API_KEY });
...
let requests = vec![
    ModelRequest::new(vec![ChatMessage::user("What is 2 + 2?")]),
    ModelRequest::new(vec![ChatMessage::user("What is the capital of France?")]),
    ModelRequest::new(vec![ChatMessage::user("Who wrote Hamlet?")]),
];
...
Pass a shared `ModelOptions` / `JsModelOptions` to apply temperature, max tokens, or a tool set to every request in the flight:

```python
from blazen import ModelOptions

result = await complete_batch(
    model,
    conversations,
    concurrency=8,
    options=ModelOptions(temperature=0.2, max_tokens=200),
)
```
...
|---|---|---|
| `responses` | `list[ModelResponse \| None]` | Per-request results in input order. |
...
The Rust version returns `BatchResult` with `responses: Vec<Result<ModelResponse, BlazenError>>` instead -- it does not split success and error into separate vectors.
...
|---|---|---|
| `.responses` | `(ModelResponse \| null)[]` | Per-request results in input order. |
...
| `.successCount` | `number` | Number of requests that produced a `ModelResponse`. |
...
- [Custom Providers](/guides/custom-providers/) -- batch also works with subclassed `Model`s
