...
// .with_store(arc_store)  // optional durable AssignmentStore (Valkey / the durable store)
...
- **Durable assignment queue** (`valkey-store` / `durable-store` features) so queued and in-flight work survives a control-plane restart; the default store is in-memory.
