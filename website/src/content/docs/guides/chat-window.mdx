...
```python
from blazen import ChatWindow, ChatMessage, OpenAiProvider, ProviderOptions
...
```typescript
import { ChatWindow, ChatMessage, OpenAiProvider } from "blazen";
...
async fn turn(
    ...
    window.add(ChatMessage::user(user_input));
    let request = ModelRequest::new(window.messages().to_vec());
