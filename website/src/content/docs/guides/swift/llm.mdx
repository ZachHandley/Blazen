...
`Model` is the chat-completion handle and `EmbeddingModel` is the embedding handle. Both are constructed via the `Providers` factory namespace and accept the same idiomatic call shapes across every provider Blazen supports.
...
`ModelRequest` carries the message list plus the usual knobs. The Swift wrapper ships an ergonomic init with sensible defaults for the rarely-used fields:
...
`Model.complete(_:)` returns a `ModelResponse`:
...
`Model.completeStream(_:)` returns an `AsyncThrowingStream<StreamEvent, Error>`. See the [Streaming guide](/guides/swift/streaming/) for the full pattern; the short version:
...
let first = try await openai.complete(
    ModelRequest(messages: messages, tools: [weatherTool])
)
...
let second = try await openai.complete(ModelRequest(messages: messages))
...
let requests = (1...10).map { i in
    ModelRequest(messages: [.user("Tell me fact #\(i)")])
}
...
Constructing a provider and calling `complete(request:)` directly is the right default. When you start juggling several models, `UniffiModelManager` is the **unified registry**: register providers — local *and* remote — under names, then dispatch by name with `complete(id:request:)` (also `stream(...)` and `get(id:)`). Each per-engine provider exposes `toModel()` to produce the `UniffiModel` the registry expects. Remote providers own no local weights, so they register with a `0` estimate and never count against a memory budget.

```swift
import BlazenSwift
import UniFFIBlazen

let manager = UniffiModelManager()
...
try await manager.registerModel(
    id: "fast",
    model: UniFFIBlazen.OpenAiProvider(apiKey: "", model: "gpt-4o-mini", baseUrl: nil).toModel(),
    memoryEstimateBytes: 0
)
try await manager.registerModel(
    id: "smart",
    model: UniFFIBlazen.AnthropicProvider(apiKey: "", model: nil, baseUrl: nil).toModel(),
    memoryEstimateBytes: 0
)

let request = ModelRequest(
    messages: [.user("Write a haiku about Swift actors.")],
    maxTokens: 256
)

// Dispatch by name — same call shape for every registered entry.
let response = try await manager.complete(id: "fast", request: request)
print(response.content)
```

For local models, `registerModel` also takes a memory-footprint estimate and the manager evicts the least-recently-used model in the same pool when a load would exceed its budget. Use `UniffiModelManager.withBudgetsGb(cpuRamGb:gpuVramGb:)` to set those budgets. See the [Local Inference](/guides/local-inference/) guide for the budgeting details.
