...
|---|---|---|---|
| `complete` | `ModelRequest` (messages + options) | `ModelResponse` | chat completion |
| `stream` | `ModelRequest` + sink | `void` (streaming chunks via sink / callback) | chat completion |
...
A completion-specific `with_before_model(hook)` typed-request hook
...
val provider = Blazen.ollama(model = "llama3.1")
    .withDefaults(ProviderDefaults(systemPrompt = "Be terse."))
```
...
- [Local Inference](/guides/local-inference/) — pair subclassing with
  `ModelManager` for per-pool memory budgeting (CPU RAM and GPU VRAM)
  on in-process providers
