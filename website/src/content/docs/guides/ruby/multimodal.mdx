...
Build a `ChatMessage` with the `media_parts:` kwarg, then pass it through `Blazen::Llm.model_request`. The `ChatMessage` constructor consumes the `Media` wrappers (it transfers their native pointers to the cabi), so you cannot reuse a `Media` instance across two messages -- build a fresh one each time.
...
req = Blazen::Llm.model_request(messages: [msg])
