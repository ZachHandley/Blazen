...
`Blazen::Streaming.complete` consumes the `ModelRequest` -- once you pass a request in, do not reuse the same wrapper for a second call. Build a fresh one each time.
