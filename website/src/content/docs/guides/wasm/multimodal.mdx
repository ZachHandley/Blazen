...
```typescript
import init, { OpenAiProvider, ChatMessage, runAgent, ContentStore, imageInput } from "@blazen/sdk";
...
export default {
  async fetch(request: Request, env: { OPENAI_API_KEY: string }): Promise<Response> {
    ...
    // Construct the provider directly, then adapt it for runAgent via toModel().
    const model = new OpenAiProvider({ apiKey: env.OPENAI_API_KEY }).toModel();
    const tools = [
      {
        name: "describe",
