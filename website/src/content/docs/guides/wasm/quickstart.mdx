...
Initialize the WASM module, then use `Model` and `ChatMessage` the same way you would in the Node.js SDK:
...
const model = Model.openai(); // reads OPENAI_API_KEY from env
