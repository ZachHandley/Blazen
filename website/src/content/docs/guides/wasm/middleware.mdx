...
The WASM SDK supports the same middleware patterns as the Node.js SDK. Each decorator method returns a **new** `Model`, keeping the original unchanged.

> The decorator chain (`withRetry()` / `withCache()` / `withFallback()`) lives on the generic `Model` type, so this guide uses the `Model.openai(...)` / `Model.openrouter(...)` factories. For a plain completion you construct a provider directly — `new OpenAiProvider({ apiKey: 'sk-...' })` — and call `.complete([...])` on it (call `.toModel()` first if you want to feed it into the decorator chain); reach for the `Model.x(...)` shorthand when you want the composable middleware surface shown below.
...
const model = Model.openai().withCache(600, 500);
```
...
```typescript
const primary = Model.openai(); // reads OPENAI_API_KEY
const backup = Model.groq();   // reads GROQ_API_KEY
...
const primary = Model.openai().withCache().withRetry();
const backup = Model.openrouter().withRetry();
