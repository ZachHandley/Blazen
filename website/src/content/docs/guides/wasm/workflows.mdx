...
```typescript
import init, { Workflow, ChatMessage, OpenRouterProvider } from '@blazen/sdk';
...
wf.addStep('summarize', ['SummarizeEvent'], async (event, ctx) => {
  ...
  // from the runtime environment -- configure the env var at deploy time
  // (see the Edge Deployment guide for strategies).
