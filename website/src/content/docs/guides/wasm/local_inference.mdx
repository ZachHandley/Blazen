...
| Method | Purpose |
|---|---|
| `register(id, sizeBytes, loader)` | Declare a model and how to load it on demand |
...
| `usedBytes()` / `availableBytes()` / `budgetBytes()` | Budget telemetry |
...
const manager = new ModelManager(512 * 1024 * 1024); // 512 MB budget

manager.register('mini-lm', 90 * 1024 * 1024, async () => {
  return await TractEmbedModel.create(miniLmOnnx, miniLmTokenizer);
});
manager.register('bge-small', 130 * 1024 * 1024, async () => {
  return await TractEmbedModel.create(bgeOnnx, bgeTokenizer);
});

const embedder = await manager.load('mini-lm');  // loads + marks MRU
console.log(manager.usedBytes(), '/', manager.budgetBytes());
```
