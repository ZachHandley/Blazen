...
3. **WebLLM** -- WebGPU-accelerated chat models via `Model.fromJsHandler()`.

## What is possible

`TractEmbedModel.create()` runs ONNX models inside the WASM module via the [tract](https://github.com/sonos/tract) inference engine. The `EmbeddingModel.fromJsHandler()` and `Model.fromJsHandler()` factories let you plug any JavaScript inference library into the Blazen pipeline. The model runs on the user's device while Blazen's `Memory`, `Model.withFallback()`, `withRetry()`, and `withCache()` work exactly the same as they do with hosted APIs.
...
```typescript
import init, { Model, ChatMessage } from '@blazen/sdk';
...
Model.fromJsHandler(
  modelId: string,
  completeHandler: (request: ModelRequest) => Promise<ModelResponse>,
  streamHandler?: (request: ModelRequest, onChunk: (chunk: StreamChunk) => void) => Promise<void>
): Model
```

The `completeHandler` receives a `ModelRequest`-shaped object and must return a `ModelResponse`-shaped object. The optional `streamHandler` enables token streaming; if omitted, `model.stream()` falls back to calling `completeHandler` and yielding the result as a single chunk.
...
```typescript
import init, { Model, OpenRouterProvider, ChatMessage } from '@blazen/sdk';
...
try {
  ...
  localModel = Model.fromJsHandler('local', localHandler);
} catch {
  // WebGPU not available or model too large
}

// Construct the provider directly, then adapt it for the Model decorator
// chain (withFallback) via toModel(). With no apiKey, it reads OPENROUTER_API_KEY.
const apiModel = new OpenRouterProvider({ apiKey: 'sk-or-...' }).toModel();

// If local model loaded, try it first; if it fails, use the API.
// If local model did not load, use the API directly.
const model = localModel
  ? Model.withFallback([localModel, apiModel])
  : apiModel;

const response = await model.complete([ChatMessage.user('Hello!')]);
```

`Model.withFallback()` is a static method that takes an array of models and tries them in order. If the first model throws a retryable error, the next is tried. Non-retryable errors (auth failures, invalid input) short-circuit.
...
```typescript
import init, { Memory, InMemoryBackend, TractEmbedModel, OpenRouterProvider, ChatMessage } from '@blazen/sdk';
...
async function answerQuestion(question: string) {
  ...
  const llm = new OpenRouterProvider({ apiKey: 'sk-or-...' });
...
```typescript
import init, { EmbeddingModel, Memory, InMemoryBackend, Model, ChatMessage } from '@blazen/sdk';
import { pipeline } from '@huggingface/transformers';
