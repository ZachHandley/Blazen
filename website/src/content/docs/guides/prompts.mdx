...
Blazen ships a lightweight prompt templating layer for reusing system prompts and user-message scaffolds across steps, agents, and workflows. Templates use `{{variable}}` placeholders, carry a chat role, and render into `ChatMessage` objects that plug straight into `Model.complete()`.
