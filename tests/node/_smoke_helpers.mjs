/**
 * Shared helpers for Node smoke tests (ava-compatible).
 *
 ...
 * diagnostics the same way: as soft passes, not failures.
 */
 ...
 * transient/infrastructure marker, call `t.pass(reason)` and return
 * `undefined`. Otherwise rethrow.
 *
 * @param {{ pass: (reason?: string) => void }} t  ava ExecutionContext (the
 *   first argument to `test(name, async (t) => { ... })`).
...
export async function falOrSkip(t, promise) {
  try {
    ...
    if (marker) {
      t.pass(`fal transient (${marker}): ${msg}`);
      return undefined;
    }
    throw e;
  }
