...
describe("SessionPausePolicy enum", () => {
  it("exposes the four documented variants as strings", () => {
    assert.strictEqual(SessionPausePolicy.PickleOrError, "PickleOrError");
    assert.strictEqual(
      SessionPausePolicy.PickleOrSerialize,
      "PickleOrSerialize",
    );
    assert.strictEqual(SessionPausePolicy.WarnDrop, "WarnDrop");
    assert.strictEqual(SessionPausePolicy.HardError, "HardError");
  });
});
...
describe("ctx.insertSessionRefSerializable / getSessionRefSerializable", () => {
  it("round-trips bytes and type name within a single step", async () => {
    ...
    wf.addStep("s", ["blazen::StartEvent"], async (event, ctx) => {
      ...
      assert.ok(payload, "expected payload to be returned");
      assert.strictEqual(payload.typeName, BLOB_TYPE_NAME);
    ...
    assert.deepEqual(result.data.recovered, { n: 42, tag: "live" });
  ...
  it("rejects malformed registry keys with a clear error", async () => {
    ...
    assert.match(result.data.error, /not-a-uuid/);
  });
});
...
describe("snapshot captures serializable session refs", () => {
  it("writes the bytes into __blazen_serialized_session_refs metadata", async () => {
    ...
    for (const [, record] of entries) {
      assert.strictEqual(record.type_tag, BLOB_TYPE_NAME);
      assert.ok(Array.isArray(record.data), "record.data should be an array");
      ...
      assert.strictEqual(embeddedTag, BLOB_TYPE_NAME);
...
describe("resumeWithSerializableRefs", () => {
  it("rehydrates the registry so getSessionRefSerializable still works", async () => {
    ...
    const snap = JSON.parse(snapJson);
    assert.ok(snap.metadata?.__blazen_serialized_session_refs);
  ...
  it("raises a clear error when a referenced type tag has no record", async () => {
